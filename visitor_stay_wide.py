...
def create_plotly_charts(visitor_data, stayed_data, congregation_averages):
    """
    Charts + bottom table (included in PNG).
    Legend moved to top-right so it never overlaps the table.

    Includes a months-based fallback gross-up for the current year's Visitor Ratio:
    - Prefer per-service counts ratio (baseline last year ÷ current year).
    - If that isn't available, use 12 ÷ full_months_elapsed (e.g., early Sept → 12/8 = 1.5).
    """
    import math
    from collections import defaultdict
    from datetime import datetime
    ...
    print("\n📊 Creating Plotly bar charts + bottom table (legend top-right)...")

    current_year = datetime.now().year
    years = [current_year - 2, current_year - 1, current_year]
    y_23, y_24, y_25 = years

    regular_services = ['8:30AM', '10:30AM', '6:30PM', 'Mid-week']
    services = regular_services + ['Overall']
    ...
    for year in years:
        ...
        for svc in regular_services:
            ...
            # --- ONLY for current year, gross-up the numerator so ratios are comparable ---
            v_for_ratio = visitors_for_ratio(
                year, svc, v, counts_map, baseline_counts_by_service
            )
            ...
            metrics[svc][year] = {'visitors': v, 'vratio': vr, 'stayed': s, 'sratio': sr}
        ...
        visitor_counts['Overall'].append(total_visitors_raw)
        stayed_counts['Overall'].append(total_stayed)
        visitor_ratios['Overall'].append((total_visitors_for_ratio / total_avg_congregation * 100) if total_avg_congregation > 0 else 0.0)
        stay_ratios['Overall'].append((total_stayed / total_visitors_raw * 100) if total_visitors_raw > 0 else 0.0)

        metrics['Overall'][year] = {
            'visitors': total_visitors_raw,
            'vratio': visitor_ratios['Overall'][-1],
            'stayed': total_stayed,
            'sratio': stay_ratios['Overall'][-1]
        }
    ...
    fig = make_subplots(
        ...
        subplot_titles=[
            ...
            'Stay Ratios (%)',
            'Values Behind the Charts (incl. YoY Δ)'
        ]
    )
    ...
    table_header = (
        ["Service"] +
        [f"{y_23} V", f"{y_23} VR%", f"{y_23} Stayed", f"{y_23} SR%"] +
        [f"{y_24} V", f"{y_24} VR%", f"{y_24} Stayed", f"{y_24} SR%"] +
        [f"{y_25} V", f"{y_25} VR%", f"{y_25} Stayed", f"{y_25} SR%"] +
        ["Δ V (25−24)", "Δ VR (pp)", "Δ Stayed (25−24)", "Δ SR (pp)"]
    )

    rows = []
    for svc in services:
        r23 = metrics[svc][y_23]; r24 = metrics[svc][y_24]; r25 = metrics[svc][y_25]
        rows.append([
            svc,
            fmt_i(r23['visitors']), fmt_p(r23['vratio']), fmt_i(r23['stayed']), fmt_p(r23['sratio']),
            fmt_i(r24['visitors']), fmt_p(r24['vratio']), fmt_i(r24['stayed']), fmt_p(r24['sratio']),
            fmt_i(r25['visitors']), fmt_p(r25['vratio']), fmt_i(r25['stayed']), fmt_p(r25['sratio']),
            fmt_i(r25['visitors'] - r24['visitors']),
            f"{(r25['vratio'] - r24['vratio']):.1f}",
            fmt_i(r25['stayed'] - r24['stayed']),
            f"{(r25['sratio'] - r24['sratio']):.1f}"
        ])

    fig.add_trace(
        go.Table(
            header=dict(values=table_header, fill_color='#e2e8f0',
                        font=dict(color='#0f172a', size=11), align='center', height=24),
            cells=dict(values=list(map(list, zip(*rows))),
                       fill_color='#ffffff', align='right', height=22,
                       font=dict(size=10, color='#334155')),
            columnwidth=[86] + [58]*12 + [82, 78, 110, 78]
        ),
        row=3, col=1
    )
    ...
    fig.update_layout(
        ...
        margin=dict(l=50, r=50, t=170, b=90)
    )

    fig.update_xaxes(showgrid=False, showline=True, linewidth=1, linecolor='#e2e8f0', tickfont=dict(size=11, color='#374151'))
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='#f1f5f9', showline=True, linewidth=1, linecolor='#e2e8f0', tickfont=dict(size=11, color='#374151'))
    fig.update_yaxes(title_text="Number of Visitors", row=1, col=1)
    fig.update_yaxes(title_text="Visitor Ratio (%)", row=1, col=2)
    fig.update_yaxes(title_text="Number Who Stayed", row=2, col=1)
    fig.update_yaxes(title_text="Stay Ratio (%)", row=2, col=2)

    return fig

def create_visitor_stay_dashboard(visitor_data, stayed_data, congregation_averages):
    """Create the complete Visitor and Stay Dashboard with charts"""
    current_year = datetime.now().year
    years = [current_year - 2, current_year - 1, current_year]
    
    print(f"\n🎨 Creating Visitor and Stay Dashboard with charts...")
    ...
    try:
        ...
        try:
            ...
            try:
                # Try alternative PNG save without auto-open
                png_filename = filename.replace('.html', '.png')
                fig.write_image(png_filename, width=1600, height=800, scale=2)
                print(f"✅ Chart saved as PNG: {png_filename}")
            except:
